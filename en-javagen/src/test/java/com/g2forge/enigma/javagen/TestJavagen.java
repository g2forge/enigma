...
public class TestJavagen extends ATestJavagen {
	...
	public void testClassFields() {
		...
		Assert.assertEquals("class MyClass {\n\t" + TestJavagen.class.getName() + " a;\n}", renderString.render(new JavaTypeDeclaration(base, "MyClass").setMembers(HCollection.asList(new JavaField(new JavaType(TestJavagen.class), "a")))));
		Assert.assertEquals("class MyClass {\n\tprotected String foo;\n\n\tprivate int bar;\n}", renderString.render(new JavaTypeDeclaration(base, "MyClass").setMembers(HCollection.asList(new JavaField(null, JavaProtection.Protected, null, new JavaType(String.class), "foo", null), new JavaField(null, JavaProtection.Private, null, new JavaType(Integer.TYPE), "bar", null)))));
	}
	...
	public void testEnum() {
		...
		Assert.assertEquals("enum MyEnum {\n\tElement;\n\n\tFoo foo;\n}", renderString.render(new JavaTypeDeclaration(null, JavaProtection.Unspecified, null, JavaMetaType.Enum, "MyEnum", null, null, HCollection.asList(new JavaElement("Element")), HCollection.asList(new JavaField(new JavaType("Foo"), "foo")))));
		Assert.assertEquals("enum MyEnum {\n\t;\n\n\tFoo foo;\n}", renderString.render(new JavaTypeDeclaration(null, JavaProtection.Unspecified, null, JavaMetaType.Enum, "MyEnum", null, null, null, HCollection.asList(new JavaField(new JavaType("Foo"), "foo")))));
	}
	...
	public void testParents() {
		...
		Assert.assertEquals("public class MyClass extends String implements ISomething {}", renderString.render(new JavaTypeDeclaration(JavaTypeDeclaration.STANDARD, "MyClass").setSuperclass(new JavaType(String.class)).setInterfaces(HCollection.asList(new JavaType("ISomething")))));
		Assert.assertEquals("public class MyClass implements ISomething {}", renderString.render(new JavaTypeDeclaration(JavaTypeDeclaration.STANDARD, "MyClass").setInterfaces(HCollection.asList(new JavaType("ISomething")))));
		Assert.assertEquals("public interface MyInterface extends ISomething0, ISomething1 {}", renderString.render(new JavaTypeDeclaration(JavaTypeDeclaration.STANDARD, "MyInterface").setMeta(JavaMetaType.Interface).setInterfaces(HCollection.asList(new JavaType("ISomething0"), new JavaType("ISomething1")))));
	}

	@Test
	public void testFile() {
		final JavaType string = new JavaType(String.class);
		final JavaMethod method = new JavaMethod(string, "toString").setStatements(HCollection.asList(new JavaVariable(JavaVariable.STANDARD, string, "retVal").setAnnotations(HCollection.asList(new JavaAnnotation(string)))));
	...
	public void testMethod() {
		...
		Assert.assertEquals("@String\nclass MyClass {\n\t@String\n\tpublic String toString() {}\n}", renderString.render(new JavaTypeDeclaration(annotations, JavaProtection.Unspecified, null, JavaMetaType.Class, "MyClass", null, null, null, HCollection.asList(new JavaMethod(string, "toString").setAnnotations(annotations).setStatements(HCollection.emptyList())))));
		Assert.assertEquals("interface MyInterface {\n\tpublic String toString();\n}", renderString.render(new JavaTypeDeclaration(null, JavaProtection.Unspecified, null, JavaMetaType.Interface, "MyInterface", null, null, null, HCollection.asList(new JavaMethod(string, "toString")))));
	}
