...
public class PUMLRenderer extends ATextualRenderer<Object, IPUMLRenderContext> {
	...
	protected static class PUMLRendering extends ARenderer.ARendering<Object, IPUMLRenderContext, IExplicitRenderable<? super IPUMLRenderContext>> {
		...
		protected void extend(TypeSwitch1.FunctionBuilder<Object, IExplicitRenderable<? super IPUMLRenderContext>> builder) {
			...
			builder.add(PUMLComponentDiagram.class, e -> c -> {
				final boolean hasComponents = e.getComponents() != null && !e.getComponents().isEmpty();
				final boolean hasLinks = e.getLinks() != null && !e.getLinks().isEmpty();

				if (hasComponents) for (PUMLComponent component : e.getComponents()) {
					c.render(component, PUMLComponent.class);
					c.newline();
				}
				if (hasComponents && hasLinks) c.newline();
				if (hasLinks) for (PUMLLink link : e.getLinks()) {
					c.render(link, PUMLLink.class);
					c.newline();
				}
			});
			...
			builder.add(PUMLLink.class, e -> c -> {
				...
				if ((e.getModifiers() != null) && !e.getModifiers().isEmpty()) {
					boolean first = true;
					c.append('[');
					for (PUMLLink.Modifier modifier : e.getModifiers()) {
						if (first) first = false;
						else c.append(',');
						c.append(modifier.name().toLowerCase());
					}
					c.append(']');
				}
				if (e.isVertical()) c.append('-');
