...
public class BashDoubleQuoteModifier implements ITextModifier, ISingleton {
	...
	public List<List<TextUpdate>> computeUpdates(List<CharSequence> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (i == 0) updates.add(new TextUpdate(0, 0, IFunction1.create(new TextCharSequence("\""))));
			if (i == list.size() - 1) updates.add(new TextUpdate(HCollection.getLast(list).length(), 0, IFunction1.create(new TextCharSequence("\""))));
